Vagrant.configure('2') do |config|
  ...
  config.vm.define :v53 do |box|
    box.vm.hostname = 'graze-signed-request-v53'
    provision_php box
    box.vm.provider :virtualbox do |vb|
      vb.customize ['modifyvm', :id, '--name', 'graze.signed-request.v53']
    end
  end

  # PHP 5.4
  config.vm.define :v54 do |box|
    box.vm.hostname = 'graze-signed-request-v54'
    box.vm.provision :shell, :inline => 'add-apt-repository ppa:ondrej/php5 && apt-get update'
    provision_php box
    box.vm.provider :virtualbox do |vb|
      vb.customize ['modifyvm', :id, '--name', 'graze.signed-request.v54']
    end
  end
